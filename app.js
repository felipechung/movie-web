  ...
  axios
    ...
    .then((response) => {
      res.render("movie", {
        ...
        movieCertification: response.data.release_dates.results,
        movieCast: response.data.credits.cast,
      });
    })
    .catch((error) => {
      console.log(error);
...
app.get("/tv", function (req, res) {
  axios
    ...
    .then((response) => {
      res.render("tv", {
        tvOnTheAir: response.data.results,
        page: response.data.page,
        pages: response.data.total_pages,
      });
    })
  ...
  axios
    .get(
      "https://api.themoviedb.org/3/tv/" +
        ...
        "?api_key=" +
        apiKey +
        "&language=en-US&append_to_response=content_ratings,credits"
    )
    .then((response) => {
      res.render("tv-show", {
        tvShowData: response.data,
        tvShowCertification: response.data.content_ratings.results,
        tvShowCast: response.data.credits.cast,
      });
    })
    .catch((error) => {
      console.log(error);
